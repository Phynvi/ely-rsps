...
public class Gambling {
	...
	public static void gambleBlackJack(Client c) {
		if(!c.getItems().playerHasItem(995, 10000)) {
			c.sendMessage("You need at least 10,000 coins to place a bet!");
			stopTheGame(c);
			return;
		}
		betting = true;
	...
	public static void blackJack(Client c) {
		if(playerBet < 10000) {
			c.sendMessage("You need to bet atleast 10,000 coins to play this game.");
			stopTheGame(c);
			return;
		}
		if(playerBet > 1000000) {
			c.sendMessage("You can only bet up to 1M coins per game.");
			stopTheGame(c);
			return;
		}
