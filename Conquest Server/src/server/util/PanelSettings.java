...
public class PanelSettings {
	...
	public String[][] MESSAGE_COLORS = {
		...
		{"Green","<col=65280>"},
		{"Yellow","<col=16776960>"},
		{"Magenta","<col=16711874>"},
		{"Orange","<col=16739072>"},
		{"Dark Red","<col=9306112>"},
		{"Dark Blue","<col=127>"}};
	...
	public void executeCommand(String cmd) {
		...
		if (p.ENTITY_LIST.getSelectedValue() != null) {
			...
			if (cmd.equalsIgnoreCase("Remove Item")) {
				...
				if (item <= 160000 && item > 0) {
					if (c.getItems().playerHasItem(item)) {
						...
						p.displayMessage("The selected item has been deleted!", cmd, 1);
					} else {
						String invItems = "";
						for (int i = 0; i < 28; i++) {
			...
			if (cmd.equalsIgnoreCase("Drop Item")) {
				...
				if (item <= 160000 && item > 0) {
					if (c.getItems().playerHasItem(item)) {
						...
						p.displayMessage("The selected item has been dropped!", cmd, 1);
					} else {
						String invItems = "";
						for (int i = 0; i < 28; i++) {
							if (c.playerItems[i] != 0) {
			...
			if (cmd.equalsIgnoreCase("Unequip Item")) {
				...
				String id = (String)JOptionPane.showInputDialog(null, "Choose the Equipment you wish to remove:\n", "Magic Bomber", -1, null, equipment, equipment);
	...
	public void playerCommand(String cmd, Client c) {
		...
		if (cmd.equals("Force Shop")) {
			int shop = -1;
			String name = (String)JOptionPane.showInputDialog(null, "Choose the Equipment you wish to remove:\n", "Magic Bomber", -1, null, Server.shopHandler.ShopName, Server.shopHandler.ShopName);
		...
		if (cmd.equals("Remove SkillXP")) {
			int id = -1;
			String name = (String)JOptionPane.showInputDialog(null, "Choose a Skill:", cmd, -1, null, SKILL_NAME, SKILL_NAME);
			int amount = getInt(cmd, "Enter the amount of Exp you want to take away:");
			for (int i = 0; i < SKILL_NAME.length; i++) {
				if(name.equals(SKILL_NAME[i]))
					id = i;
			}
	...
	public static class Location {
		...
		public static Location getLocationByName(String name) {
			if(name.equals("Home"))
				return new Location(2844, 10209, 0);
