...
getGameComponent().setCursor(getGameComponent().getToolkit().createCustomCursor(image, new Point(0, 0), null));
	...
	public void mouseWheelMoved(MouseWheelEvent e) {
		...
		if (mouseY > 210 && mouseY < 473 && mouseX > 514 && mouseX < 762) {//Quest	
			if(client.tabInterfaceIDs[client.tabID] == 638)
				RSInterface.interfaceCache[29160].scrollPosition += notches*30;	
		}
		...
		if (mouseX > 0 && mouseX < 512 && mouseY > 0 && mouseY < 334) {//Ingame
			if(client.openInterfaceID == 23000)
				RSInterface.interfaceCache[5385].scrollPosition += notches*30;
		}
	}
	/*public void mouseWheelMoved(MouseWheelEvent e) {
		int notches = e.getWheelRotation();
		if (saveClickX > 0 && saveClickY > 340 && saveClickX < 510 && saveClickY < 500) {
			client.anInt1089 -= notches*30;		
		} else if (saveClickY > 210 && saveClickY < 473 && saveClickX > 514 && saveClickX < 762) {
			int[][] data = {
				{638, 29160},
				{29265, 29268},
				{18128, 18143},
			...
			for(int i = 0; i < data.length; i++) {
				if(client.tabInterfaceIDs[client.tabID] == data[i][0]) {
  					RSInterface.interfaceCache[data[i][1]].scrollPosition += notches*30;
				}
			}
		} else if (saveClickX > 0 && saveClickX < 512 && saveClickY > 0 && saveClickY < 334) {
			if(client.openInterfaceID == 5292 || client.openInterfaceID == 23000) {
				RSInterface.interfaceCache[5385].scrollPosition += notches*30;
			}
		}
	}*/
