...
public class ModelDecompressor {
	...
	public static void loadHeads() {
		try {
			DataInputStream indexFile = new DataInputStream(new FileInputStream(signlink.findcachedir() + "/660NPCHeads.idx"));
			DataInputStream dataFile = new DataInputStream(new FileInputStream(signlink.findcachedir() + "/660NPCHeads.dat"));
			int length = indexFile.readInt();
			for(int i = 0; i < length; i++) {
				int id = indexFile.readInt();
				int invlength = indexFile.readInt();
